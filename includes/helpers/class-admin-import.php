						...
						if ( $updated ) {
							...
							if ( isset( $OSProjectsProject ) && method_exists( $OSProjectsProject, 'update_project_meta_fields' ) ) {
								...
								if ( $post_status === 'ignored' && ! empty( $git_error ) ) {
									...
									$error_messages[] = sprintf( __( '%1$s - Repository marked as ignored: %2$s', 'osprojects' ), esc_html( $repo_url ), esc_html( $git_error ) )
										. ' ' . OSProjectsProject::project_action_link( $project_id, 'edit', '_blank' );
									++$error_count;
								} else {
		...
		<div class="wrap">
			...
			<form method="post" action="">
				...
				echo esc_html(
					sprintf(
						__(
							'Select the repositories you wish to import from %s.',
							'osprojects'
						),
						$github_user_url,
					)
						...
						foreach ( $repositories as $repo ) :
							...
							<tr>
								<th scope="row" class="check-column">
									<input type="checkbox" name="selected_repos[]" value="<?php echo esc_attr( base64_encode( wp_json_encode( $repo ) ) ); ?>" 
									<?php
									if ( $project_id ) {
										echo 'disabled';}
									?>
