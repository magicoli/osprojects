...
class OSProjectsProject {
	...
	public function text_to_blocks( $text ) {
		...
		$segments = preg_split( '/\n\s*\n/', $text );
		$blocks   = array_map(
			function ( $segment ) use ( $parsedown ) {
	...
	public static function enqueue_refresh_queue( $post_ids = null ) {
		...
		if ( $total < 100 ) {
			...
			if ( $batch_size > 10 ) {
				$batch_size = 10; // maximum 10 per batch
			}
			if ( $batch_size < 1 ) {
				$batch_size = 1;
			}
	...
	public static function process_refresh_queue_batch( $batch_size = 10 ) {
		...
		foreach ( $batch as $i => $post_id ) {
			...
			$progress_now['current_project'] = array(
				'post_id' => $post_id,
				'title'   => $title,
				'index'   => $index,
			);
			...
			try {
				...
				if ( empty( $repo_url ) ) {
					...
					if ( ! isset( $p['failed_items'] ) ) {
						$p['failed_items'] = array();
					}
					$p['failed_items'][] = array(
						'post_id' => $post_id,
						'title'   => get_the_title( $post_id ),
				...
				if ( ! isset( $p['processed_items'] ) ) {
					$p['processed_items'] = array();
				}
				$p['processed_items'][] = array(
					'post_id' => $post_id,
					'title'   => $title,
				...
				if ( $i + 1 < $batch_count ) {
					...
					$p['status']          = 'processing';
					$p['current_project'] = array(
						'post_id' => $next_id,
						'title'   => $next_title,
						'index'   => $p['processed'] + $p['failed'] + 1,
					);
		...
		if ( empty( $queue ) ) {
			...
			$progress['current_project'] = array(
				'post_id' => $next_id,
				'title'   => $next_title,
				'index'   => (int) $progress['processed'] + (int) $progress['failed'] + 1,
			);
		...
		if ( ! isset( $progress['failed_items'] ) ) {
